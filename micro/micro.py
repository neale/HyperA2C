...
def train():
    ...
    bs = args.batch_size
    agent_memory = Variable(torch.zeros(args.batch_size, 256)).cuda(), Variable(torch.zeros(args.batch_size, 256)).cuda()
    ...
    while info['frames'][0] <= 4e7:
        ...
        episode_length +=1
        #run agent on state
        value, logit, agent_memory = agent((state, agent_memory))
        ...
        if done_count > 0:
            ...
            for j, d in enumerate(done):
                if d:
                    ...
                    #agent_memory[0][j], agent_memory[1][j] = 0, 0
        ...
        if i % args.lstm_steps == 0:
            next_value = agent((state, agent_memory))[0]
            if done_count > 0: 
                for j, d in enumerate(done):
                    if d: next_value[j] = 0
